...
computer_execute :: proc(c: ^Computer, operation: Operation) {
	...
	case Operation_Skip_Pressed:
		key := c.registers[op.register_key] & 0xF
		if c.keys[key] == 1 {
			c.program_counter += 2
		}
	...
	case Operation_Index_Sprite:
		char := c.registers[op.register_sprite] & 0xF
	...
	case Operation_Draw:
		...
		for row in 0 ..< op.bytes {
			...
			sprite_row: u8 = c.memory[(c.index_register + u16(row)) & 0xFFF]
	...
	case Operation_Convert_Decimal:
		...
		c.memory[c.index_register & 0xFFF] = hundreds
		c.memory[(c.index_register + 1) & 0xFFF] = decimals
		c.memory[(c.index_register + 2) & 0xFFF] = units
	case Operation_Store_Registers:
		for r in 0 ..= op.register_to {
			c.memory[(c.index_register + u16(r)) & 0xFFF] = c.registers[r]
		}
	...
	case Operation_Load_Registers:
		for r in 0 ..= op.register_to {
			c.registers[r] = c.memory[(c.index_register + u16(r)) & 0xFFF]
		}
		if Quirk.Memory in c.quirks {
			c.index_register += 1 + u16(op.register_to)
		}
